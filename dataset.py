...
from KNN_adjacency import calcADJ
...
class ViT_HER2ST(torch.utils.data.Dataset):
    ...
    def __init__(self, train=True, sr=False, fold=0):
...
class ViT_DLPC(torch.utils.data.Dataset):

    def __init__(self, train=True, gene_list=None, ds=None, sr=False, aug=False, norm=False, fold=0):
        super(ViT_DLPC, self).__init__()
        self.dir = './data/10X'
        self.r = 224 // 4
        ...
        gene_list = list(np.load('./data/dlpc_hvg_cut_1000.npy'))
    ...
    def get_img(self, name):
        path = glob.glob(self.dir + '/' + name + '/spatial/' + name + '_full_image.tif')[0]
        im = Image.open(path)
        return im

    def get_cnt(self, name):
        path = glob.glob(self.dir + '/' + name + '/*' + '_stdata.csv')[0]
        df = pd.read_csv(path, sep=',', index_col=0)
        return df
    ...
    def get_pos(self, name):
        ...
        # pattern = f"{self.dir}/*spot*{name}*.tsv"
        path = glob.glob(self.dir + '/' + name + '/spatial' + '/' + 'tissue_positions_list.csv')[0]
        print(path)
        df = pd.read_csv(path, sep=',')
    ...
    def get_meta(self, name, gene_list=None):
        cnt = self.get_cnt(name)
        pos = self.get_pos(name)
        # meta = cnt.join((pos.set_index('spot_name')))
        meta = cnt.join(pos.set_index('spot_name'), how='inner')
        return meta
        # path = glob.glob(self.dir + '/*' + name + '_stdata.tsv')[0]
        # meta = pd.read_csv('./data/10X/151507/10X_Visium_151507_stdata.csv', index_col=0)
        # return meta
...
class ViT_MOB(torch.utils.data.Dataset):

    def __init__(self, train=True, gene_list=None, ds=None, sr=False, aug=False, norm=False, fold=0):
        super(ViT_MOB, self).__init__()
        self.dir = './data/MOB'
        self.r = 224 // 4
        # sample_names = ['Rep1_MOB', 'Rep2_MOB', 'Rep3_MOB', 'Rep4_MOB', 'Rep5_MOB', 'Rep6_MOB', 'Rep7_MOB', 'Rep8_MOB',
        #                 'Rep9_MOB', 'Rep10_MOB', 'Rep11_MOB', 'Rep12_MOB']
        sample_names = ['Rep1_MOB', 'Rep2_MOB']

        gene_list = list(
            np.load('data/mob_hvg_cut_1000.npy', allow_pickle=True))
    ...
    def get_cnt(self, name):
        path = glob.glob(self.dir + '/*' + name + '_count_matrix-1_modified.tsv')[0]
        df = pd.read_csv(path, sep='\t', index_col=0)
        return df

    def get_pos(self, name):

        pattern = f"{self.dir}/{name}_spot_coordinate.tsv"
        path = glob.glob(pattern)[0]
        print(path)
        df = pd.read_csv(path, sep='\t')
